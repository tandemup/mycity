...
getGroups() {
  ...
  return this.firestore.collection<any>('userProfile', ref => ref.where('permission', '==', 'retrailer')).snapshotChanges().pipe(
    map(actions => {
      return actions.map(a => {
        const data = a.payload.doc.data();
        // get id from firebase metadata
...
getGroupDetail(groupId: string) {
  ...
  return this.firestore.doc<any>('userProfile/' + groupId).valueChanges();
